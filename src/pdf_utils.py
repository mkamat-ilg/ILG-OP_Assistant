...
# Use PyMuPDF for PDF rendering (works on Streamlit Cloud without poppler)
import fitz  # pymupdf


def pdf_bytes_to_images(pdf_bytes: bytes, max_pages: int = 12, dpi: int = 150) -> List[bytes]:
    """
    Render up to max_pages pages from a PDF into PNG bytes.
    """
...
def image_bytes_to_images(img_bytes: bytes) -> List[bytes]:
    """
    Treat a single image as a 1-page document.
    Normalizes into PNG bytes.
    """
...
def render_doc_viewer(file_obj: Dict[str, Any], key_prefix: str):
    """
    Scroll + zoom viewer.
    - PDFs: render pages as images and show in a scrollable column.
    - Images: show a single image.
    """
    ...
    if ext == "pdf":
        imgs = pdf_bytes_to_images(b, max_pages=int(max_pages), dpi=int(72 * (zoom / 100.0) * 2))  # simple scaling
